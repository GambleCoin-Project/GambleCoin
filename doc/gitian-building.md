...
*Setup instructions for a gitian build of GambleCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GambleCoin
...
- [Building GambleCoin](#building-gamblecoin)
...
Clone the git repositories for gamblecoin and gitian and then checkout the gamblecoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/gamblecoin-crypto/gamblecoin.git
cd gamblecoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the gamblecoin repository
...
To build GambleCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the gamblecoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gamblecoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/gamblecoin-crypto/gamblecoin
...
For example:
```bash
URL=https://github.com/crowning-/gamblecoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gamblecoin=${COMMIT} --url gamblecoin=${URL} ../gamblecoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gamblecoin=${COMMIT} --url gamblecoin=${URL} ../gamblecoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gamblecoin=${COMMIT} --url gamblecoin=${URL} ../gamblecoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gamblecoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gamblecoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/gamblecoin-build.assert
```
...
[gamblecoin/gitian.sigs](https://github.com/gamblecoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
